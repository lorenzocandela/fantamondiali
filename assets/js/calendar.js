...
export function generateRoundRobin(teams) {
    ...
    for (let r = 0; r < rounds; r++) {
        ...
        for (let i = 0; i < total / 2; i++) {
            ...
            if (home.uid !== 'bye' && away.uid !== 'bye') round.push({
                home: home.uid, away: away.uid, home_name: home.team_name, away_name: away.team_name
            });
        }
...
<p>L'admin deve generarlo dalla dashboard</p>
        ...
        document.getElementById('cal-matches-list').innerHTML = `<div class="empty-state"><span class="material-symbols-outlined">wifi_off</span><h3>Errore</h3><p>${err.message}</p></div>`;
    }
}
...
async function openMatchDetail(homeUid, awayUid, round) {
    ...
    if (status === 'future' && isMine) { mdView = 'formazione'; } else { mdView = 'confronto'; }
    ...
    if (userSnap.exists()) {
        ...
        lineup = savedIds.length ? savedIds.map(id => roster.find(p => String(p.id) === String(id))).filter(Boolean) : autoFill(roster, activeModule);
        roster.forEach(p => {
...
`<button class="form-module-chip ${m === activeModule ? 'active' : ''}" data-module="${m}">${m}</button>`
    ...
    const rows = [{ role: 'ATT', count: att }, { role: 'CEN', count: cen }, { role: 'DIF', count: dif }, { role: 'POR', count: 1 }];
...
${Array.from({ length: row.count }).map((_, i) => slotHtml(rp[i] ?? null, row.role, i, false)).join('')}
</div>`;
}).join('')}
</div>
</div>`;
...
<button class="fpicker-close" id="fpicker-close"><span class="material-symbols-outlined">close</span></button>
</div>
${currentId ? `
...
<img class="fpicker-photo" src="${p.photo ?? ''}" alt="${p.name}" onerror="this.src='https://placehold.co/44x44/f2f2f7/aeaeb2?text=${encodeURIComponent(p.name?.[0]??'?')}'">
...
async function saveFormazione() {
    ...
    try {
        ...
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<span class="material-symbols-outlined">check_circle</span>Salva formazione G${mdRound}`;
        }
    }
}
...
function autoFill(players, mod) {
    ...
    players.forEach(p => {
        if (!filled.find(f => String(f.id) === String(p.id)) && !bench.find(b => String(b.id) === String(p.id))) bench.push(p);
    });
    return [...filled, ...bench];
}
...
function calcLiveScore(player, stats) {
    ...
    let score = base;
    score += (stats.goals ?? 0) * (SCORE_TABLE.goal[player.role] ?? 3);
...
function bonusBreakdownLine(player, stats, totalScore) {
    ...
    if (g > 0) {
        ...
        parts.push(`<span class="bd-chip bd-goal${finClass}"><span class="material-symbols-outlined">sports_soccer</span>${g > 1 ? g + '×' : ''}+${val}</span>`);
    }
    if (a > 0) {
        const val = a * SCORE_TABLE.assist;
        parts.push(`<span class="bd-chip bd-assist${finClass}"><span class="material-symbols-outlined">handshake</span>${a > 1 ? a + '×' : ''}+${val}</span>`);
    }
    if (r > 0) {
        parts.push(`<span class="bd-chip bd-red${finClass}"><span class="material-symbols-outlined">square</span>${SCORE_TABLE.red}</span>`);
    } else if (y > 0) {
...
async function fetchLiveScores() {
    try {
        ...
        const res = await fetch(`script/get_live_scores.php?mode=test&date=${dateStr}`);
...
<span>${m.home_name}</span><span class="admin-cal-vs">vs</span><span>${m.away_name}</span>
</div>`).join('')}
</div>`).join('');
...
document.getElementById('btn-reset-calendar')?.addEventListener('click', async () => {
    ...
    btn.disabled = true;
    try {
        await setDoc(doc(db, 'settings', 'calendar'), { schedule: [], results: {}, generated_at: null, });
        previewSchedule = [];
        document.getElementById('admin-cal-preview')?.classList.add('hidden');
...
<button class="admin-module-del" data-module="${m}"><span class="material-symbols-outlined">close</span></button>
</div>`).join('') : `<div style="font-size:12px;color:var(--text-3)">Nessun modulo custom aggiunto</div>`;
