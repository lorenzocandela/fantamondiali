...
export function getCurrentMatchday() {
    ...
    for (let i = 0; i < MATCHDAY_SCHEDULE.length - 1; i++) {
        ...
        if (today > endCur && today < startNxt) return {
            ...MATCHDAY_SCHEDULE[i + 1],
            status: 'next'
        };
    }
    return {
        ...MATCHDAY_SCHEDULE[MATCHDAY_SCHEDULE.length - 1],
        status: 'ended'
    };
}
// ─── ADMIN MATCHDAY ──────────────────────────────────────────────────────────
    ...
    try {
        ...
        usersSnap.forEach(d => {
            ...
            if (data.competition_joined) calTeams.push({
                uid: d.id,
                team_name: data.team_name ?? 'Squadra',
...
document.getElementById('cal-round-next')?.addEventListener('click', () => {
    if (calRound < calSchedule.length) {
        calRound++;
        renderCalRound();
    }
});
...
${isCurrent ? 'attuale' : isStarter ? 'titolare' : 'panchina'}
    ...
    sheet.querySelectorAll('.fpicker-row').forEach(row => {
        row.addEventListener('click', () => {
            applyPick(String(row.dataset.id));
            closePicker();
        });
    });
}
...
function closePicker() {
    ...
    overlay.addEventListener('transitionend', () => overlay.classList.add('hidden'), {
        once: true
    });
    pickerState = null;
}
...
async function saveFormazione() {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<span class="material-symbols-outlined">hourglass_empty</span> Salvataggio...';
    }
    try {
...
function validateLineup() {
    ...
    if (pors === 0) w.push({
        icon: 'error',
        text: 'Nessun portiere schierato',
        level: 'error'
    });
    if (pors > 1) w.push({
        icon: 'warning',
        text: 'Più di un portiere tra i titolari',
        level: 'warn'
    });
    if (difs < dif) w.push({
        icon: 'warning',
        text: `Modulo ${activeModule}: servono ${dif} DIF (hai ${difs})`,
        level: 'warn'
    });
    if (cens < cen) w.push({
        icon: 'warning',
        text: `Modulo ${activeModule}: servono ${cen} CEN (hai ${cens})`,
        level: 'warn'
    });
    if (atts < att) w.push({
        icon: 'warning',
        text: `Modulo ${activeModule}: servono ${att} ATT (hai ${atts})`,
        level: 'warn'
    });
    if (tot < 11) w.push({
        icon: 'info',
        text: `${tot}/11 titolari schierati`,
        level: 'info'
    });
    return w;
}
...
function calcLiveScore(player, stats) {
    ...
    if (!stats.rating || stats.rating === 0) {
        if (stats.sub_in != null && stats.sub_in >= 75) {
            const hasBonus = (stats.goals ?? 0) > 0 || (stats.assists ?? 0) > 0 || (stats.yellow ?? 0) > 0 || (stats.red ?? 0) > 0 || stats.cs;
...
function renderPlayerCell(p, stats, score, side, pending, isDetail) {
    ...
    const subHtml = subOutMin != null ? `<span class="confronto-sub sub-out" title="Uscito ${subOutMin}'">

<span class="material-symbols-outlined">arrow_downward</span>${subOutMin}'

</span>` : subInMin != null ? `<span class="confronto-sub sub-in" title="Entrato ${subInMin}'">
...
function buildFullSchedule(teams) {
    ...
    if (base.length >= 7) return base.slice(0, 7).map((rd, i) => ({
        ...rd,
        round: i + 1
    }));
    const full = [];
...
document.getElementById('btn-add-module')?.addEventListener('click', async () => {
    ...
    if (!val) return;
    if (!/^\d+-\d+-\d+(-\d+)?$/.test(val)) {
        toast('Formato non valido — es. 4-3-3', 'error');
        return;
    }
    const snap = await getDoc(doc(db, 'settings', 'modules'));
    const list = snap.exists() ? (snap.data().list ?? []) : [];
    if (list.includes(val)) {
        toast('Modulo già presente', 'error');
        return;
    }
