...
$searchDate  = $_GET['date'] ?? date('Y-m-d'); // Di default cerca le partite di oggi
...
if ($fixtureId) {
    // ==========================================
    // MODO 1: DEBUG PARTITA SINGOLA (3 Colonne)
    // ==========================================
    $fixtureReq = apiGet("https://v3.football.api-sports.io/fixtures?id={$fixtureId}");
    $rawLineups = apiGet("https://v3.football.api-sports.io/fixtures/lineups?fixture={$fixtureId}");
    $rawEvents  = apiGet("https://v3.football.api-sports.io/fixtures/events?fixture={$fixtureId}");

    $fixtureData = $fixtureReq['response'][0] ?? null;
    $lineups     = $rawLineups['response'] ?? [];
    $events      = $rawEvents['response'] ?? [];

    // MOTORE FANTACALCIO (Calcolo Voti)
    if (!empty($lineups)) {
        foreach ($lineups as $team) {
            foreach ($team['startXI'] as $p) { $playerScores[$p['player']['id']] = 6.0; }
            foreach ($team['substitutes'] as $p) { $playerScores[$p['player']['id']] = null; }
        }
    }
    ...
    // ==========================================
    // MODO 2: RICERCA PARTITE
    // ==========================================
    $url = "https://v3.football.api-sports.io/fixtures?date={$searchDate}";
    $partite = apiGet($url);
    $allMatches = $partite['response'] ?? [];

    if (!empty($searchQuery)) {
        // Filtra per squadra se l'utente ha scritto qualcosa
...
<head>
    ...
    <title>Ultimate Fanta-Debug</title>
...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-slate-800 rounded-2xl p-4 border border-slate-700 shadow flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <?php else: ?>
        <a href="?" class="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-2">
            ⬅ Torna alla Ricerca
        </a>
        <?php endif; ?>
    </div>
    ...
    <?php if (!$fixtureId): ?>
        <div class="bg-slate-800 rounded-2xl border border-slate-700 p-6 shadow-inner min-h-[50vh]">
            <h2 class="text-xl font-bold mb-4">Risultati per il <?= date('d/m/Y', strtotime($searchDate)) ?> <?= $searchQuery ? " - Ricerca: \"$searchQuery\"" : '' ?></h2>
            
            <?php if (empty($matchesList)): ?>
                <div class="text-center text-slate-500 py-10">Nessuna partita trovata per questi criteri.</div>
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <?php foreach ($matchesList as $m): ?>
                    <div class="bg-slate-900 border border-slate-700 p-4 rounded-xl flex flex-col justify-between hover:border-blue-500 transition">
                        <div class="flex justify-between text-xs text-slate-400 mb-2 pb-2 border-b border-slate-800">
                            ...
                            <span class="font-bold text-<?= $m['fixture']['status']['short'] === 'FT' ? 'emerald' : 'yellow' ?>-500">
                        ...
                        <a href="?fixture=<?= $m['fixture']['id'] ?>" class="mt-4 block text-center bg-blue-600/20 text-blue-400 border border-blue-600/50 hover:bg-blue-600 hover:text-white py-2 rounded-lg text-sm font-bold transition">
                            Debug Partita (ID: <?= $m['fixture']['id'] ?>)
                        </a>
                    </div>
                    <?php endforeach; ?>
                </div>
    ...
    <?php if ($fixtureId && $fixtureData): ?>
        ...
        <div class="bg-slate-800 rounded-2xl shadow-2xl p-6 border border-slate-700 flex flex-col md:flex-row items-center justify-between">
            ...
            <div class="flex flex-col items-center w-1/3">
                ...
                <span class="text-xs text-slate-400 mt-2"><?= $fixtureData['league']['name'] ?></span>
            </div>
                        ...
                        <div class="flex items-center gap-4 bg-slate-800 p-3 rounded-xl border border-slate-700/50 relative overflow-hidden">
                            ...
                            <div class="flex-1 min-w-0 z-10">
                                <p class="font-bold text-slate-200 truncate">
                                    <?= $ev['player']['name'] ?? 'Allenatore / Panchina' ?>
